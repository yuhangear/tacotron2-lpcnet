...
n_average=5 # if > 0, the model averaged with n_average ckpts will be used instead of model.loss.best
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    local/data_prep.sh biaobei_data data/${trans_type}_train ${trans_type}
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    (
        ...
        convert_fbank.sh --nj ${nj} --cmd "${cmd}" \
            --fs ${fs} \
            --fmax "${fmax}" \
            --fmin "${fmin}" \
            --n_fft ${n_fft} \
